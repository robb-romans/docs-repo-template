...
*(Cloud DNS)* Domain Name System. Determines Internet :ref:`domain <domain-term>`
...
*(Cloud DNS)* A record that belongs to a particular :ref:`domain <domain-term>` and is
used to specify information about the . There are several types
...
*(Cloud DNS)* The account that creates the :ref:`domain-term`.
...
method for :ref:`fine-grained access control <fine-grain-access-control>`).
...
*(Cloud DNS)* A :ref:`domain <domain-term>` within a parent domain that cannot be
