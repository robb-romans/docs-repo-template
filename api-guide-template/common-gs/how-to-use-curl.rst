...
The following example shows a cURL command for sending an authentication request
to the Rackspace Cloud Identity service.
...
about creating environment variables, see :ref:`Configure environment
...
.. note::
   ...
   If your request includes the ``-i`` option to show header output, do not
   try to pretty-print the output. Header information is not in JSON format,
   and the API service returns an error if you specify json.tool.
...
- Replace all the line continuation backslash characters (``\``) with a
  caret (``^``), and remove any trailing spaces after the ``^``.
